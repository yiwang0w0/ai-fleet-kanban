---
name: context-window
description: 查看各线会话的上下文窗口占用,判断是否需要压缩(compact)。Check per-line session context usage; when (not) to compact.
---

# 上下文窗口查看(context-window)

操作者问「上下文还剩多少 / 哪条线快满了 / 要不要压缩」时用本 skill。
...
- **压缩会改写提示词前缀 → 提示缓存变冷 → 下一次调用显著变贵。**
  实测结论(见 `loops/worker_loop.py` 内的记账注释):缓存热时长上下文便宜,
  贵的是冷掉的前缀。所以压缩不是保养,是有代价的手术。
- 只在两个条件同时成立时压缩:①窗口临近上限 ②这条线接下来还要继续长跑。
- 正道是面板「上下文」区的压缩按钮(它带鉴权、发事件、落记录)。
- 对没有持续会话的线压缩会得到 400「没有持续会话可压缩」——这是正常回答,不是故障。
