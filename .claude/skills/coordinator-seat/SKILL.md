...
- 你刚完成部署(`npm start` 起来、`/health` 绿、面板地址已交给操作者),或
- 操作者说「你来管看板 / 接管看板」,或
...
操作者在面板「请你的 Claude 代劳」行(v0.18 前叫「协调席快捷指令」)按了按钮 → server 落一行请求 → 你的 SSE 哨打出
...
**「CLI 参数契约」**那一项。它跑真的 `claude --help`,把循环实际传的 12 个参数
