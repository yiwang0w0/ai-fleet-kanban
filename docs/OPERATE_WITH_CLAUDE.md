...
「跑着的是哪一版、磁盘上已经是哪一版」,并只列还差的步骤——重新验收、重启看板、
重挂两哨(哨是独立进程,看板重启不会更新它们;它们会自报版本,所以板知道)。
底栏一直显示当前跑着的版本号。三步都给命令不给按钮:验收是你的动作,重启会打断
在飞的活。

## 二点一、面板顶上的上手引导(v0.6)

第一次开板,面板顶部是一条**引导**:六步,每步一句「现在该做什么」,当前那步高亮
并给出办法——能一键的就一个按钮(只有「生成配置文件」是一键:纯创建、无治理含义),
该你跑命令的给命令+复制,该协调席做的一键交给它。走完变成一行「配置完成 ✓」。

每一步的状态都是**当场测出来的**,不是打过勾就记住:删掉配置、改了代码没重新验收、
哨断了——引导会自己退回那一步。测不出来的显示「?」而不是「已完成」。
