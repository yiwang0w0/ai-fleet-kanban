...
`lines[]{id,hint,label?,accept?}` (`label` = display name, `accept` = `human` default / `auto` = deliveries complete on report through the same close gates; v0.21) · `roles[]` (`review` only — reorg retired by ruling 2026-09-02, before the freeze bound it; joins only when its loop script
